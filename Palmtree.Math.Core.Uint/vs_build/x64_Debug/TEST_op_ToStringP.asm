...
__B66AD74D_test_op_tostringp@c DB 01H
msvcjmc	ENDS
PUBLIC	TEST_PMC_ToStringP
PUBLIC	__JustMyCode_Default
PUBLIC	??_C@_1EK@HDEICNAJ@?$AAP?$AAM?$AAC?$AA_?$AAF?$AAr?$AAo?$AAm?$AAB?$AAy?$AAt?$AAe?$AAA?$AAr?$AAr@ ; `string'
PUBLIC	??_C@_1CM@DNOIAFFH@?$AAP?$AAM?$AAC?$AA_?$AAT?$AAo?$AAS?$AAt?$AAr?$AAi?$AAn?$AAg?$AAP?$AA?5?$AA?$CI@ ; `string'
...
_BSS	SEGMENT
?actual_str_buffer@?1??TEST_PMC_ToStringP@@9@9 DW 01000H DUP (?) ; `TEST_PMC_ToStringP'::`2'::actual_str_buffer
_BSS	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$TEST_PMC_ToStringP DD imagerel $LN10
	DD	imagerel $LN10+644
	DD	imagerel $unwind$TEST_PMC_ToStringP
pdata	ENDS
;	COMDAT rtc$TMZ
rtc$TMZ	SEGMENT
...
??_C@_1EA@HONIIPBB@?$AAP?$AAM?$AAC?$AA_?$AAT?$AAo?$AAS?$AAt?$AAr?$AAi?$AAn?$AAg?$AAn?$PP?$KJ?$AA0@ DB 'P'
	...
	DB	00H						; `string'
CONST	ENDS
;	COMDAT ??_C@_1CM@DNOIAFFH@?$AAP?$AAM?$AAC?$AA_?$AAT?$AAo?$AAS?$AAt?$AAr?$AAi?$AAn?$AAg?$AAP?$AA?5?$AA?$CI@
CONST	SEGMENT
??_C@_1CM@DNOIAFFH@?$AAP?$AAM?$AAC?$AA_?$AAT?$AAo?$AAS?$AAt?$AAr?$AAi?$AAn?$AAg?$AAP?$AA?5?$AA?$CI@ DB 'P'
	DB	00H, 'M', 00H, 'C', 00H, '_', 00H, 'T', 00H, 'o', 00H, 'S', 00H
	DB	't', 00H, 'r', 00H, 'i', 00H, 'n', 00H, 'g', 00H, 'P', 00H, ' '
...
;	COMDAT xdata
xdata	SEGMENT
$unwind$TEST_PMC_ToStringP DD 035054a19H
	DD	011d3322H
	DD	070160047H
	DD	05015H
...
;	COMDAT CONST
CONST	SEGMENT
TEST_PMC_ToStringP$rtcName$0 DB 078H
	DB	00H
	ORG $+2
TEST_PMC_ToStringP$rtcName$1 DB 06fH
	DB	070H
	DB	074H
	DB	00H
	ORG $+8
TEST_PMC_ToStringP$rtcVarDesc DD 0a0H
	DD	0b0H
	DQ	FLAT:TEST_PMC_ToStringP$rtcName$1
	DD	038H
	DD	08H
	DQ	FLAT:TEST_PMC_ToStringP$rtcName$0
	ORG $+96
TEST_PMC_ToStringP$rtcFrameData DD 02H
	DD	00H
	DQ	FLAT:TEST_PMC_ToStringP$rtcVarDesc
CONST	ENDS
...
; File z:\sources\lunor\repos\rougemeilland\palmtree.math.core.uint\palmtree.math.core.uint\test_op_tostringp.c
;	COMDAT TEST_PMC_ToStringP
_TEXT	SEGMENT
x$ = 8
result$ = 36
x_result$ = 68
...
group_separator$ = 584
group_sizes$ = 592
decimal_separator$ = 600
percent_symbol$ = 608
decimal_digits$ = 616
pattern$ = 624
desired_str$ = 632
TEST_PMC_ToStringP PROC					; COMDAT
...
$LN10:
	...
	lea	rcx, OFFSET FLAT:__B66AD74D_test_op_tostringp@c
...
; 40   :     lstrcpyW(opt.Percent.GroupSeparator, group_separator);

	mov	rdx, QWORD PTR group_separator$[rbp]
	lea	rcx, QWORD PTR opt$[rbp+106]
	call	QWORD PTR __imp_lstrcpyW

; 41   :     lstrcpyW(opt.Percent.GroupSizes, group_sizes);

	mov	rdx, QWORD PTR group_sizes$[rbp]
	lea	rcx, QWORD PTR opt$[rbp+112]
	call	QWORD PTR __imp_lstrcpyW

; 42   :     lstrcpyW(opt.Percent.DecimalSeparator, decimal_separator);

	mov	rdx, QWORD PTR decimal_separator$[rbp]
	lea	rcx, QWORD PTR opt$[rbp+100]
	call	QWORD PTR __imp_lstrcpyW

; 43   :     lstrcpyW(opt.PercentSymbol, percent_symbol);

	mov	rdx, QWORD PTR percent_symbol$[rbp]
	lea	rcx, QWORD PTR opt$[rbp+162]
	call	QWORD PTR __imp_lstrcpyW

; 44   :     opt.Percent.DecimalDigits = decimal_digits;

	mov	eax, DWORD PTR decimal_digits$[rbp]
	mov	DWORD PTR opt$[rbp+96], eax

; 45   :     opt.Percent.PositivePattern = pattern;

	mov	eax, DWORD PTR pattern$[rbp]
	mov	DWORD PTR opt$[rbp+140], eax

; 46   :     TEST_Assert(env, FormatTestLabel(L"PMC_ToStringP (%d.%d)", no, 1), (x_result = ep->From_L(x_value, &x)) == PMC_STATUS_OK, FormatTestMesssage(L"PMC_FromByteArrayの復帰コードが期待通りではない(%d)", x_result));
...
$LN5@TEST_PMC_T:
	...
	lea	rcx, OFFSET FLAT:??_C@_1CM@DNOIAFFH@?$AAP?$AAM?$AAC?$AA_?$AAT?$AAo?$AAS?$AAt?$AAr?$AAi?$AAn?$AAg?$AAP?$AA?5?$AA?$CI@
	call	FormatTestLabel
	mov	rcx, QWORD PTR tv66[rbp]
	mov	r9, rcx
	mov	r8d, DWORD PTR tv83[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 47   :     TEST_Assert(env, FormatTestLabel(L"PMC_ToStringP (%d.%d)", no, 2), (result = ep->ToString(x, format, &opt, actual_str_buffer, sizeof(actual_str_buffer))) == PMC_STATUS_OK, FormatTestMesssage(L"PMC_ToStringの復帰コードが期待通りではない(%d)", result));

	mov	QWORD PTR [rsp+32], 8192		; 00002000H
	lea	r9, OFFSET FLAT:?actual_str_buffer@?1??TEST_PMC_ToStringP@@9@9
...
; 48   :     TEST_Assert(env, FormatTestLabel(L"PMC_ToStringP (%d.%d)", no, 3), lstrcmpW(actual_str_buffer, desired_str) == 0, L"データの内容が一致しない");

	mov	rdx, QWORD PTR desired_str$[rbp]
	lea	rcx, OFFSET FLAT:?actual_str_buffer@?1??TEST_PMC_ToStringP@@9@9
...
; 51   : }
	...
	lea	rdx, OFFSET FLAT:TEST_PMC_ToStringP$rtcFrameData
