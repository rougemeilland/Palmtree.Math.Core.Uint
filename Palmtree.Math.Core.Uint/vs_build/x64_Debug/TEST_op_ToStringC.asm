...
__A80ACA64_test_op_tostringc@c DB 01H
msvcjmc	ENDS
PUBLIC	TEST_PMC_ToStringC
PUBLIC	__JustMyCode_Default
PUBLIC	??_C@_1EK@HDEICNAJ@?$AAP?$AAM?$AAC?$AA_?$AAF?$AAr?$AAo?$AAm?$AAB?$AAy?$AAt?$AAe?$AAA?$AAr?$AAr@ ; `string'
PUBLIC	??_C@_1CM@HCIHODLI@?$AAP?$AAM?$AAC?$AA_?$AAT?$AAo?$AAS?$AAt?$AAr?$AAi?$AAn?$AAg?$AAC?$AA?5?$AA?$CI@ ; `string'
...
_BSS	SEGMENT
?actual_str_buffer@?1??TEST_PMC_ToStringC@@9@9 DW 01000H DUP (?) ; `TEST_PMC_ToStringC'::`2'::actual_str_buffer
_BSS	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$TEST_PMC_ToStringC DD imagerel $LN10
	DD	imagerel $LN10+635
	DD	imagerel $unwind$TEST_PMC_ToStringC
pdata	ENDS
;	COMDAT rtc$TMZ
rtc$TMZ	SEGMENT
...
??_C@_1EA@HONIIPBB@?$AAP?$AAM?$AAC?$AA_?$AAT?$AAo?$AAS?$AAt?$AAr?$AAi?$AAn?$AAg?$AAn?$PP?$KJ?$AA0@ DB 'P'
	...
	DB	00H						; `string'
CONST	ENDS
;	COMDAT ??_C@_1CM@HCIHODLI@?$AAP?$AAM?$AAC?$AA_?$AAT?$AAo?$AAS?$AAt?$AAr?$AAi?$AAn?$AAg?$AAC?$AA?5?$AA?$CI@
CONST	SEGMENT
??_C@_1CM@HCIHODLI@?$AAP?$AAM?$AAC?$AA_?$AAT?$AAo?$AAS?$AAt?$AAr?$AAi?$AAn?$AAg?$AAC?$AA?5?$AA?$CI@ DB 'P'
	DB	00H, 'M', 00H, 'C', 00H, '_', 00H, 'T', 00H, 'o', 00H, 'S', 00H
	DB	't', 00H, 'r', 00H, 'i', 00H, 'n', 00H, 'g', 00H, 'C', 00H, ' '
...
;	COMDAT xdata
xdata	SEGMENT
$unwind$TEST_PMC_ToStringC DD 035054a19H
	DD	011d3322H
	DD	070160047H
	DD	05015H
...
;	COMDAT CONST
CONST	SEGMENT
TEST_PMC_ToStringC$rtcName$0 DB 078H
	DB	00H
	ORG $+2
TEST_PMC_ToStringC$rtcName$1 DB 06fH
	DB	070H
	DB	074H
	DB	00H
	ORG $+8
TEST_PMC_ToStringC$rtcVarDesc DD 0a0H
	DD	0b0H
	DQ	FLAT:TEST_PMC_ToStringC$rtcName$1
	DD	038H
	DD	08H
	DQ	FLAT:TEST_PMC_ToStringC$rtcName$0
	ORG $+96
TEST_PMC_ToStringC$rtcFrameData DD 02H
	DD	00H
	DQ	FLAT:TEST_PMC_ToStringC$rtcVarDesc
CONST	ENDS
...
; File z:\sources\lunor\repos\rougemeilland\palmtree.math.core.uint\palmtree.math.core.uint\test_op_tostringc.c
;	COMDAT TEST_PMC_ToStringC
_TEXT	SEGMENT
x$ = 8
result$ = 36
x_result$ = 68
...
pattern$ = 624
desired_str$ = 632
TEST_PMC_ToStringC PROC					; COMDAT
...
$LN10:
	...
	lea	rcx, OFFSET FLAT:__A80ACA64_test_op_tostringc@c
...
; 40   :     lstrcpyW(opt.Currency.GroupSeparator, group_separator);

	mov	rdx, QWORD PTR group_separator$[rbp]
	lea	rcx, QWORD PTR opt$[rbp+10]
	call	QWORD PTR __imp_lstrcpyW

; 41   :     lstrcpyW(opt.Currency.GroupSizes, group_sizes);

	mov	rdx, QWORD PTR group_sizes$[rbp]
	lea	rcx, QWORD PTR opt$[rbp+16]
	call	QWORD PTR __imp_lstrcpyW

; 42   :     lstrcpyW(opt.Currency.DecimalSeparator, decimal_separator);

	mov	rdx, QWORD PTR decimal_separator$[rbp]
	lea	rcx, QWORD PTR opt$[rbp+4]
	call	QWORD PTR __imp_lstrcpyW

; 43   :     lstrcpyW(opt.CurrencySymbol, currency_symbol);

	mov	rdx, QWORD PTR currency_symbol$[rbp]
	lea	rcx, QWORD PTR opt$[rbp+144]
	call	QWORD PTR __imp_lstrcpyW

; 44   :     opt.Currency.DecimalDigits = decimal_digits;

	mov	eax, DWORD PTR decimal_digits$[rbp]
	mov	DWORD PTR opt$[rbp], eax

; 45   :     opt.Currency.PositivePattern = pattern;

	mov	eax, DWORD PTR pattern$[rbp]
	mov	DWORD PTR opt$[rbp+44], eax

; 46   :     TEST_Assert(env, FormatTestLabel(L"PMC_ToStringC (%d.%d)", no, 1), (x_result = ep->From_L(x_value, &x)) == PMC_STATUS_OK, FormatTestMesssage(L"PMC_FromByteArrayの復帰コードが期待通りではない(%d)", x_result));
...
$LN5@TEST_PMC_T:
	...
	lea	rcx, OFFSET FLAT:??_C@_1CM@HCIHODLI@?$AAP?$AAM?$AAC?$AA_?$AAT?$AAo?$AAS?$AAt?$AAr?$AAi?$AAn?$AAg?$AAC?$AA?5?$AA?$CI@
...
; 47   :     TEST_Assert(env, FormatTestLabel(L"PMC_ToStringC (%d.%d)", no, 2), (result = ep->ToString(x, format, &opt, actual_str_buffer, sizeof(actual_str_buffer))) == PMC_STATUS_OK, FormatTestMesssage(L"PMC_ToStringの復帰コードが期待通りではない(%d)", result));

	mov	QWORD PTR [rsp+32], 8192		; 00002000H
	lea	r9, OFFSET FLAT:?actual_str_buffer@?1??TEST_PMC_ToStringC@@9@9
...
; 48   :     TEST_Assert(env, FormatTestLabel(L"PMC_ToStringC (%d.%d)", no, 3), lstrcmpW(actual_str_buffer, desired_str) == 0, L"データの内容が一致しない");

	mov	rdx, QWORD PTR desired_str$[rbp]
	lea	rcx, OFFSET FLAT:?actual_str_buffer@?1??TEST_PMC_ToStringC@@9@9
...
; 51   : }
	...
	lea	rdx, OFFSET FLAT:TEST_PMC_ToStringC$rtcFrameData
